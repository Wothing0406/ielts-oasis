  ...
  return (
    <section className="xl:col-span-5 matcha-card p-8 bento-card">
      <div className="flex items-center justify-between mb-8">
        ...
        <button type="button" className="text-xs font-bold text-primary bg-primary/10 px-4 py-2 rounded-full hover:bg-primary/20 transition-all">
          Browse All
        </button>
      </div>
      
      <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
        {vocabList.length > 0 ? vocabList.slice(0, 5).map((item) => (
          <div key={item.id || item.word} className="p-4 bg-secondary dark:bg-neutral-800 rounded-medium flex items-center justify-between group">
      ...
      <div className="mt-6 flex flex-col gap-3">
        <input 
          aria-label="Type a word to brew"
          type="text" 
